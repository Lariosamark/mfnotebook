...
import {
  ...
  MousePointer, ChevronDown, RotateCcw, Pencil, Square, Minus, MoreHorizontal
} from 'lucide-react'
...
export default function DiagramCanvas({ data, onChange, readOnly, style }) {
  ...
  const onTouchMove = (e) => {
    ...
    if (e.touches.length === 1) {
      const t = e.touches[0]
      if (panning) {
        setPan({ x: panning.px + (t.clientX - panning.sx), y: panning.py + (t.clientY - panning.sy) })
      }
      if (isDrawing.current && activeStroke) {
        const pt = toSvg(t.clientX, t.clientY)
        if (activeStroke.type === 'pen') {
  ...
  const setModeAndReset = (m) => {
    ...
    if (m === 'pen' || m === 'box' || m === 'line') setShowColorRow(true)
    else setShowColorRow(false)
  }
      ...
      {!readOnly && (
        <div className="flex-shrink-0 bg-gray-50 border-b border-gray-200">
          ...
          <div className="flex items-center gap-1 px-2 py-1.5 flex-wrap">
            ...
            {/* Mode tools */}
            <button onClick={() => setModeAndReset('select')}
              title="Select (V)"
            ...
            <button
              ...
              className={`${toolBtnCls('connect')} flex items-center gap-1`}>
              <Link2 className="w-3.5 h-3.5" />
              {mode === 'connect' && (
                <span className="text-xs font-medium hidden sm:inline">
                  {connectSrc ? 'Target…' : 'Source…'}
                </span>
              )}
            </button>

            <button onClick={() => setModeAndReset('pen')}  title="Freehand draw (P)" className={toolBtnCls('pen')}>
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setModeAndReset('box')}  title="Draw box (B)"      className={toolBtnCls('box')}>
              <Square className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setModeAndReset('line')} title="Draw line (L)"     className={toolBtnCls('line')}>
              <Minus className="w-3.5 h-3.5" />
            </button>

            {/* Color swatch toggle — only shown in draw modes, mobile */}
            {isDrawMode && (
              <button
            ...
            {/* Color swatches — always visible on sm+ */}
            {isDrawMode && (
            ...
            <button
              onClick={() => { setNodes([]); setEdges([]); setDrawings([]); setSelected(null) }}
          ...
          {/* Expandable colour row — mobile only, draw modes */}
