    ...
    <div className="flex flex-col h-full overflow-hidden bg-[#f7f8fc]">
      ...
      <div className="md:hidden flex items-center gap-2 px-3 py-2 border-b border-gray-200 bg-white flex-shrink-0">
        ...
        {(selectedPage || selectedFolder) && (
          <button
            onClick={() => { setSelectedPage(null); setSelectedFolder(null) }}
            className="ml-auto text-xs text-gray-500 hover:text-gray-800 px-2 py-1 rounded-lg hover:bg-gray-100 flex-shrink-0">
            ← Back
          </button>
        )}
      </div>
    ...
    <div className="h-full overflow-y-auto p-3 sm:p-5">
      <div className="max-w-4xl mx-auto">
        <div className="mb-4">
          ...
          <p className="text-gray-400 text-xs sm:text-sm mt-0.5">System overview and quick access</p>
        </div>
              ...
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex items-center gap-3 px-4 py-3 animate-pulse">
                    ...
                    <div className="flex-1"><div className="h-3 bg-gray-200 rounded w-32 mb-1.5" /><div className="h-2.5 bg-gray-100 rounded w-48" /></div>
              ...
              {loading ? (
                Array.from({ length: 4 }).map((_, i) => (
                  <div key={i} className="flex items-center gap-3 animate-pulse">
                    <div className="w-8 h-8 rounded-full bg-gray-200 flex-shrink-0" />
                    <div className="flex-1"><div className="h-3 bg-gray-200 rounded w-28 mb-1.5" /><div className="h-2 bg-gray-100 rounded w-20" /></div>
...
function UserDetailPanel({ user, notebooks, folders, tab, onTabChange, onSelectPage, onSelectFolder,
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-3 sm:px-5 py-3 sm:py-4">
        <div className="flex items-start gap-3">
          <Avatar name={user.full_name} size="lg" />
          <div className="flex-1 min-w-0">
            <h2 className="text-base sm:text-lg font-bold text-gray-900 truncate">{user.full_name}</h2>
        ...
        <div className="flex items-center gap-1 mt-3 bg-gray-100 rounded-xl p-1 w-fit max-w-full">
          ...
          <TabBtn active={tab==='folders'} onClick={() => onTabChange('folders')}
            icon={<Folder className="w-3.5 h-3.5"/>} label={`Folders (${folders.length})`} />
        </div>
      </div>
...
function TabBtn({ active, onClick, icon, label }) {
  return (
    <button onClick={onClick} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all whitespace-nowrap ${
      ...
      {notebooks.map(nb => (
        <div key={nb.id} className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div onClick={() => onToggleNotebook(nb)}
            className={`flex items-center gap-3 px-3 sm:px-4 py-3 cursor-pointer hover:bg-gray-50 transition-colors ${nbExpanded[nb.id] ? 'border-b border-gray-100' : ''}`}>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {nbSections[nb.id] && <span className="text-xs text-gray-400 hidden sm:inline">{nbSections[nb.id].length} sections</span>}
          ...
          {nbExpanded[nb.id] && (
            <div className="px-2 sm:px-3 py-2 space-y-0.5 bg-gray-50/50">
                            ...
                            : (sectionPagesMap[s.id]||[]).length === 0
                              ...
                              : (sectionPagesMap[s.id]||[]).map(p => (
                                  <div key={p.id} onClick={() => onSelectPage(p)}
                                    ...
                                    <span className="text-xs text-gray-300 group-hover:text-gray-500 flex-shrink-0">View →</span>
                                  </div>
                                ))
                          }
                        </div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      {folders.map(f => (
        <div key={f.id} onClick={() => onSelectFolder(f)}
          className="bg-white border border-gray-200 rounded-2xl p-3 sm:p-4 cursor-pointer hover:border-amber-300 hover:shadow-md transition-all group shadow-sm">
          <div className="w-9 h-9 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center mb-2.5 group-hover:bg-amber-100 transition-colors">
            <Folder className="w-4 h-4 text-amber-600"/>
          </div>
          <p className="font-semibold text-gray-800 text-sm truncate mb-0.5">{f.site_name}</p>
          ...
          <p className="text-xs text-gray-300 mt-1.5">{formatDate(f.updated_at)}</p>
        </div>
      ))}
    </div>
  )
...
function AdminPageView({ page, comments, commentText, setCommentText, onSendComment,
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      {/* Top bar */}
      <div className="flex-shrink-0 bg-white border-b border-gray-200 px-3 py-2 flex items-center gap-1.5 overflow-x-auto">
        <button onClick={onBack}
          className="hidden md:flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-800 font-medium px-2 py-1 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0">
          ← Back
        </button>
        <span className="hidden md:block text-gray-300 flex-shrink-0">|</span>
      ...
      {/* Mobile tab bar */}
      <div className="md:hidden flex border-b border-gray-100 flex-shrink-0 bg-gray-50">
        <button onClick={() => setMobileTab('editor')}
          className={`flex-1 py-2.5 text-xs font-semibold transition-colors ${mobileTab === 'editor' ? 'text-violet-700 border-b-2 border-violet-600 bg-white' : 'text-gray-400'}`}>
          📄 Page
        </button>
        <button onClick={() => setMobileTab('comments')}
          className={`flex-1 py-2.5 text-xs font-semibold transition-colors relative ${mobileTab === 'comments' ? 'text-violet-700 border-b-2 border-violet-600 bg-white' : 'text-gray-400'}`}>
          💬 Comments
          {comments.length > 0 && (
            <span className="ml-1 inline-flex items-center justify-center w-4 h-4 rounded-full bg-violet-500 text-white text-[9px] font-bold">{comments.length}</span>
          )}
        </button>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        {/* Editor panel */}
        <div className={`flex-1 overflow-hidden min-w-0 ${mobileTab !== 'editor' ? 'hidden md:block' : ''}`}>
          <NoteEditor key={`${page.id}-${editorMode}`} content={page.content}
            onChange={editorMode ? onUpdatePage : undefined} readOnly={!editorMode}/>
        </div>

        {/* Comments panel */}
        <div className={`md:w-72 lg:w-80 flex-shrink-0 flex flex-col bg-white border-t md:border-t-0 md:border-l border-gray-200 ${mobileTab !== 'comments' ? 'hidden md:flex' : 'flex w-full'}`}>
...
function AdminFolderViewer({ folder, fetchImages, onBack }) {
  ...
  return (
    <div className="flex flex-col h-full animate-fade-in">
      <div className="px-3 sm:px-5 py-3 bg-white border-b border-gray-200 flex items-center gap-2 flex-shrink-0">
        <button onClick={onBack}
          className="hidden md:flex items-center gap-1.5 text-xs text-gray-500 hover:text-gray-800 font-medium px-2 py-1 rounded-lg hover:bg-gray-100 transition-colors flex-shrink-0">
          ← Back
        </button>
        <span className="hidden md:block text-gray-300">|</span>
        <div className="w-7 h-7 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center flex-shrink-0">
          <FolderOpen className="w-3.5 h-3.5 text-amber-600"/>
        </div>
        <div className="flex-1 min-w-0">
          <h2 className="font-bold text-gray-800 text-sm truncate">{folder.site_name}</h2>
          <p className="text-xs text-gray-400 flex items-center gap-1"><MapPin className="w-3 h-3 flex-shrink-0"/><span className="truncate">{folder.location}</span></p>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4">
        {loading ? (
          ...
          <div className="space-y-2">
            {images.map(img => (
              <div key={img.id} onClick={() => setLightbox(img)}
                className="flex items-center gap-3 p-2.5 sm:p-3 bg-white border border-gray-200 rounded-xl hover:border-blue-300 transition-colors cursor-pointer">
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-3 sm:p-8" onClick={() => setLightbox(null)}>
          <button className="absolute top-3 right-3 text-white/70 hover:text-white p-2 rounded-full hover:bg-white/10"><X className="w-6 h-6"/></button>
          <div className="max-w-4xl max-h-full flex flex-col items-center gap-3" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-white font-medium text-center text-sm">{lightbox.file_name}</p>
          </div>
        </div>
      )}
    </div>
...
function BigStatCard({ icon, label, value, color }) {
  ...
  return (
    <div className={`${c} border rounded-2xl p-3 sm:p-4 flex flex-col gap-1.5 sm:gap-2 shadow-sm`}>
